...
class TabReformerModelTest(tf.test.TestCase):

  class TabReformerModelTester(object):
    ...
    def create_model(self):
      input_ids = TabReformerModelTest.ids_tensor([self.batch_size, self.seq_length],
                                           self.vocab_size)

      input_mask = None
      if self.use_input_mask:
        input_mask = TabReformerModelTest.ids_tensor(
            [self.batch_size, self.seq_length], vocab_size=2)

      token_type_ids = None
      if self.use_token_type_ids:
        token_type_ids = TabReformerModelTest.ids_tensor(
            [self.batch_size, self.seq_length], self.type_vocab_size)

      config = modeling.TabReformerConfig(
  ...
  def test_default(self):
    self.run_tester(TabReformerModelTest.TabReformerModelTester(self))

  def test_config_to_json_string(self):
    config = modeling.TabReformerConfig(vocab_size=99, hidden_size=37)
